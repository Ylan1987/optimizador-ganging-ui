...
// TAREA 4: Añadido FileText para el ícono de CSV
...
// #################################################################################
// Este componente ahora es accesible por teclado y corrige el problema de edición.
// - Se puede enfocar con la tecla Tab.
// - Al estar enfocado, presionar "Enter" o "Espacio" activa el modo de edición.
// - Esto soluciona el problema en los "price brackets" y mejora la usabilidad en toda la app.
const EditableField = ({ value, onChange, placeholder, type = "text", onStartEdit, onEndEdit, isEditing, options, className = "" }) => {
    const handleKeyDown = (e) => {
        if (e.key === 'Enter' || e.key === 'Escape') {
            (e.target).blur(); // Causa que se llame a onEndEdit
        }
    };

    const handleViewKeyDown = (e) => {
        if (e.key === 'Enter' || e.key === ' ') {
            e.preventDefault(); // Previene el scroll en caso de la barra espaciadora
            onStartEdit();
        }
    };

    const commonClasses = `w-full bg-transparent px-1 py-0.5 rounded-sm transition-colors text-white/90 text-sm ${className}`;
    const viewClasses = `border-b border-transparent hover:border-white/30 focus:outline-none focus:border-cyan-500 cursor-pointer`;
    const editClasses = `border-b border-cyan-500 focus:border-cyan-400 focus:outline-none bg-black/30`;

    if (type === 'select') {
        return (
            <div className="relative w-full">
                <select value={value?? ""} onChange={onChange} onBlur={onEndEdit} onKeyDown={handleKeyDown} className={`${commonClasses} ${editClasses} h-[28px] appearance-none custom-select pr-6`}>
    ...
    if (isEditing) {
        return <input type={type} value={value?? ""} onChange={onChange} onBlur={onEndEdit} onKeyDown={handleKeyDown} placeholder={placeholder} className={`${commonClasses} ${editClasses}`} autoFocus />;
    }

    return (
        <div
            className={`${commonClasses} ${viewClasses} min-h-[28px] flex items-center`}
            onClick={onStartEdit}
            onKeyDown={handleViewKeyDown}
            tabIndex={0} // TAREA 2: Hace que el div sea enfocable con el teclado
            role="button" // TAREA 2: Rol semántico para accesibilidad
...
const ImpositionPage = ({ supabase, onSelectQuote }) => {
    ...
    // Efecto para la búsqueda inicial y cuando cambia el término de búsqueda
    useEffect(() => {
    ...
    // Efecto para cargar más páginas cuando el número de página cambia
    useEffect(() => {
        if (page > 1) {
    ...
    // Callback ref para el observador de intersección
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {searchResults.map((quote, index) => {
                    if (searchResults.length === index + 1) {
                        return <div ref={lastElementRef} key={quote.id}><SearchResultCard quote={quote} onSelect={onSelectQuote} /></div>;
                    }
...
const JobsInputPage = ({ onOptimize, materialsData }) => {
    ...
    const addJob = () => {
        const newJob = { id: `job-${Date.now()}`, name: `Trabajo Nuevo ${jobs.length + 1}`, width: 148, length: 210, quantity: 1000, bleed: 3, rotatable: true, samePlatesForBack: false, material: { name: '', grammage: '' }, frontInks: 4, backInks: 0 };
        setJobs(p => [newJob,...p]);
    };
    ...
    const handleUploadAccepted = (results) => {
        ...
        for (let i = 1; i < results.data.length; i++) {
            ...
            const newJob = {
                ...
                bleed: toNum(jobData.bleed)?? 3,
                rotatable: jobData.rotatable? jobData.rotatable.toLowerCase() === 'true' : true,
                samePlatesForBack: jobData.samePlatesForBack? jobData.samePlatesForBack.toLowerCase() === 'true' : false,
                material: { name: jobData.material_name, grammage: toNum(jobData.grammage) },
        ...
        setJobs(prevJobs => [...newJobs.reverse(),...prevJobs]);
    };

    return (
        <div className="p-4 space-y-6">
            <header className="flex flex-wrap items-center gap-3">
                <h1 className="text-2xl font-bold mr-auto">Cotizador de Trabajos</h1>
                <button title="Agregar Trabajo" onClick={addJob} className="p-2 rounded bg-cyan-600/20 border border-cyan-500/30 text-cyan-300 hover:bg-cyan-600/40 transition-colors font-semibold flex items-center gap-2">
                    ...
                    {({ getRootProps, acceptedFile, ProgressBar, getRemoveFileProps }) => (
                        <>
                            <div className="flex items-center gap-4">
                                <button type='button' {...getRootProps()} className="px-4 py-2 bg-slate-700 text-white font-semibold rounded-lg hover:bg-slate-600 transition-colors">
                                    Seleccionar CSV
                                </button>
            ...
            <div className="space-y-4">
                {jobs.map(job => {
                    ...
                    const grammageOptions = selectedMaterial? selectedMaterial.grades.flatMap(g => g.grams).map(gr => ({ value: gr, label: `${gr}g` })) :;
                    return (
                        <div key={job.id} className="rounded-xl border border-gray-700 bg-slate-800/50 p-4">
                            <div className="flex items-center justify-between gap-2 border-b border-gray-700 pb-3 mb-4">
                                ...
                                <IconButton title="Eliminar Trabajo" onClick={() => removeJob(job.id)} colorClass="text-red-500"><Trash2 size={18} /></IconButton>
                            </div>
                            <div className="flex flex-row flex-wrap gap-x-4 gap-y-3 items-end">
                                <LabeledField label="Ancho (mm)" className="w-20"><EditableField type="number" value={job.width} onChange={e => updateJob(job.id, { width: toNum(e.target.value) })} isEditing={editingField === `${job.id}-w`} onStartEdit={() => setEditingField(`${job.id}-w`)} onEndEdit={() => setEditingField(null)} /></LabeledField>
                                <LabeledField label="Largo (mm)" className="w-20"><EditableField type="number" value={job.length} onChange={e => updateJob(job.id, { length: toNum(e.target.value) })} isEditing={editingField === `${job.id}-l`} onStartEdit={() => setEditingField(`${job.id}-l`)} onEndEdit={() => setEditingField(null)} /></LabeledField>
                                <LabeledField label="Sangrado (mm)" className="w-24"><EditableField type="number" value={job.bleed} onChange={e => updateJob(job.id, { bleed: toNum(e.target.value) })} isEditing={editingField === `${job.id}-bleed`} onStartEdit={() => setEditingField(`${job.id}-bleed`)} onEndEdit={() => setEditingField(null)} /></LabeledField>
                                <LabeledField label="Cantidad" className="w-20"><EditableField type="number" value={job.quantity} onChange={e => updateJob(job.id, { quantity: toNum(e.target.value) })} isEditing={editingField === `${job.id}-q`} onStartEdit={() => setEditingField(`${job.id}-q`)} onEndEdit={() => setEditingField(null)} /></LabeledField>
                                <LabeledField label="Material" className="w-40"><EditableField type="select" value={job.material.name} onChange={e => updateJob(job.id, { material: { name: e.target.value, grammage: '' } })} options={} onEndEdit={() => { }} /></LabeledField>
                                <LabeledField label="Gr." className="w-24"><EditableField type="select" value={job.material.grammage} onChange={e => updateJob(job.id, { material: {...job.material, grammage: toNum(e.target.value) } })} options={[{ value: '', label: '...' },...grammageOptions]} onEndEdit={() => { }} /></LabeledField>
                                <LabeledField label="Tintas F." className="w-20"><EditableField type="number" value={job.frontInks} onChange={e => updateJob(job.id, { frontInks: toNum(e.target.value) })} isEditing={editingField === `${job.id}-f`} onStartEdit={() => setEditingField(`${job.id}-f`)} onEndEdit={() => setEditingField(null)} /></LabeledField>
                                <LabeledField label="Tintas D." className="w-20"><EditableField type="number" value={job.backInks} onChange={e => updateJob(job.id, { backInks: toNum(e.target.value) })} isEditing={editingField === `${job.id}-b`} onStartEdit={() => setEditingField(`${job.id}-b`)} onEndEdit={() => setEditingField(null)} /></LabeledField>
            ...
            {jobs.length > 0 && (
                <div className="mt-6 flex justify-end">
                    <button onClick={() => onOptimize(jobs)} className="px-8 py-3 bg-cyan-600 text-white font-bold rounded-lg hover:bg-cyan-700 transition-colors flex items-center gap-2">
                        <Wand2 size={20} /> Optimizar Cotización
                    </button>
                </div>
            )}
        </div>
...
const adaptApiResponse = (apiResult) => {
    const adaptLayout = (layout) => {
        ...
        if (layout.placements && Array.isArray(layout.placements)) {
            layout.placements = layout.placements.map(p => ({...p, w: p.width, h: p.length }));
        }
...
export default function App() {
        ...
        try {
            ...
            for (const layout of Object.values(layouts)) {
                for (const jobInLayout of layout.jobsInLayout) {
                    ...
                    jobsForApi.push({
                        job_name: jobName,
                        trim_box: { width: jobDetails.width, height: jobDetails.length, bleed: jobDetails.bleed },
            ...
            const layoutData = {
                sheet_config: Object.values(layouts).pressSheetSize,
                jobs: jobsForApi,
            };
            ...
            const response = await fetch(`${pythonApiUrl}/generate-imposition`, {
                method: 'POST',
                headers: { 'x-vercel-protection-bypass': '9NcyUFK5OAlsMPdCOKD9FgttJzd9G7Op' },
                body: formData
            });
            if (!response.ok) {
    ...
    const handleSaveQuote = async (quoteNumber, solutionData, totalCost) => {
        setLoading(true);
        try {
            const { data: existing, error: checkError } = await supabase.from('quotes').select('id').eq('quote_number', quoteNumber).single();
            ...
            const payload = {
                ...
                ganging_result: optimizationResult,
                chosen_solution_name: solutionData.summary? solutionData.layouts.id].layoutId : Object.keys(solutionData.layouts),
    ...
    const handleOptimize = async (jobs) => {
        ...
        try {
            ...
            const finalResultWithJobs = {...adaptedResult, jobs: apiPayload.jobs };
            setOptimizationResult(finalResultWithJobs);
            setCurrentPage('workspace');
        } catch (error) {
            console.error("Error calling optimizer API:", error);
            alert("Error al llamar al optimizador: " + error.message);
        } finally {
            setLoading(false);
        }
    };
