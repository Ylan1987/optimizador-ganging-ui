...
const EditableField = ({ value, onChange, placeholder, type = "text", onStartEdit, onEndEdit, isEditing, options, className = "" }) => {
    ...
    const viewClasses = `border-b border-transparent hover:border-white/30 cursor-pointer focus:outline-none focus:border-cyan-400 focus:bg-black/30`;
    ...
    return (
        <div
            ...
            onClick={onStartEdit}
            onFocus={onStartEdit} // Activa la edición cuando el elemento recibe foco (al tabular)
            tabIndex={0}         // Hace que el div sea navegable con la tecla Tab
        >
...
const MachinesAdminPage = ({ initialData, onSave }) => {
    ...
    const addBracket = (mi) => {
        const next = {
            id: `bracket-${Date.now()}`, // ID único para el nuevo tramo
            constraints: { maxLen: 0, maxWid: 0 },
            sheetCost: { unit: "per_sheet", value: 0 }
        };
        mut(mi, { price_brackets: [ ...(items[mi].price_brackets || []), next] });
    };
...
const JobsInputPage = ({ onOptimize, materialsData }) => {
    ...
    const newJob = {
        ...
        length: 210,
        quantity: 1000,
        bleed: 3, // <--- AÑADIDO: Sangrado por defecto de 3mm
    ...
    /**
     * Parsea el contenido de texto de un CSV y lo convierte en objetos de trabajo,
     * actualizando el estado de la aplicación.
     * @param {string} csvText - El contenido del archivo CSV.
     *
     * Formato esperado del CSV (las columnas pueden estar en cualquier orden):
     * name,width,length,quantity,bleed,material_name,material_grammage,frontInks,backInks,rotatable,samePlatesForBack
     */
    const parseAndSetJobs = (csvText) => {
        try {
            const lines = csvText.trim().split(/\r\n|\n/);
            if (lines.length < 2) {
                alert("Error: El archivo CSV está vacío o no contiene datos.");
                return;
            }
            const header = lines[0].split(',').map(h => h.trim());
            const expectedHeaders = ['name', 'width', 'length', 'quantity', 'bleed', 'material_name', 'material_grammage', 'frontInks', 'backInks', 'rotatable', 'samePlatesForBack'];
            
            // Verificación simple de que todas las cabeceras esperadas existen
            if (!expectedHeaders.every(h => header.includes(h))) {
                alert(`Error: El CSV debe contener las siguientes columnas obligatorias: ${expectedHeaders.join(', ')}`);
                return;
            }
            ...
            const newJobs = lines.slice(1).map((line, index) => {
                ...
                const row = header.reduce((obj, key, i) => {
                    obj[key] = values[i] ? values[i].trim() : '';
                    return obj;
                }, {});

                return {
                    id: `csv-job-${Date.now()}-${index}`,
                    name: row.name || `Trabajo CSV ${index + 1}`,
                    width: Number(row.width) || 0,
                    length: Number(row.length) || 0,
                    quantity: Number(row.quantity) || 0,
                    bleed: Number(row.bleed) || 3,
                    frontInks: Number(row.frontInks) || 4,
                    backInks: Number(row.backInks) || 0,
                    rotatable: row.rotatable ? row.rotatable.toLowerCase() === 'true' : true,
                    samePlatesForBack: row.samePlatesForBack ? row.samePlatesForBack.toLowerCase() === 'true' : false,
                    material: {
                        name: row.material_name || '',
                        grammage: Number(row.material_grammage) || ''
                    }
                };
            }).filter(Boolean); // Filtrar cualquier línea nula

            if (newJobs.length > 0) {
                 setJobs(prevJobs => [...newJobs, ...prevJobs]); // Añade los nuevos trabajos al principio de la lista
            } else {
                alert("No se pudieron leer trabajos válidos del archivo CSV.");
            }
        } catch (error) {
            console.error("Error al procesar el CSV:", error);
            alert("Ocurrió un error al procesar el archivo CSV. Revisa la consola para más detalles.");
        }
    };
    return ( <div className="p-4 space-y-6"> <header className="flex flex-wrap items-center gap-3"> <h1 className="text-2xl font-bold mr-auto">Cotizador de Trabajos</h1> 
        <label className="cursor-pointer p-2 rounded bg-green-600/20 border border-green-500/30 text-green-300 hover:bg-green-600/40 transition-colors font-semibold flex items-center gap-2">
            <FileUp size={20}/> Subir CSV
            <input type="file" accept=".csv" className="hidden" onChange={handleFileChange} />
        </label>
        <button title="Agregar Trabajo" onClick={addJob} className="p-2 rounded bg-cyan-600/20 border border-cyan-500/30 text-cyan-300 hover:bg-cyan-600/40 transition-colors font-semibold flex items-center gap-2"><Plus size={20}/> Agregar Trabajo</button> </header> <div className="space-y-4"> {jobs.map(job => { const selectedMaterial = materialsData.find(m => m.name === job.material.name); const grammageOptions = selectedMaterial ? selectedMaterial.grades.flatMap(g => g.grams).map(gr => ({value: gr, label: `${gr}g`})) : []; return ( <div key={job.id} className="rounded-xl border border-gray-700 bg-slate-800/50 p-4"> <div className="flex items-center justify-between gap-2 border-b border-gray-700 pb-3 mb-4"> <EditableField value={job.name} onChange={e => updateJob(job.id, {name: e.target.value})} isEditing={editingField === `${job.id}-name`} onStartEdit={() => setEditingField(`${job.id}-name`)} onEndEdit={() => setEditingField(null)} className="!text-lg !font-semibold" /> <IconButton title="Eliminar Trabajo" onClick={() => removeJob(job.id)} colorClass="text-red-500"><Trash2 size={18}/></IconButton> </div> 
